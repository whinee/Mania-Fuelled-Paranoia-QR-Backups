\subsection{Key Concepts}

\begin{description}[leftmargin=!,labelwidth=6em]
  \item[Arguments] Group of statements, one of which is claimed to follow from the others.
  \item[Proposition] A statement that is either true or false, usually a declarative sentence.
\end{description}

\subsection{Logic Connectives}

\begin{table}[H]
\centering
\caption{Logic Connectives}
\begin{tabular}[t]{|c|c|c|}
\hline
\textbf{Connectives} & \textbf{Symbols} & \textbf{Meaning} \\ \hline
Negation & $\sim$ / $\lnot$ & Not \\ \hline
Conjunction & $\land$ & And \\ \hline
Disjunction & $\lor$ & Or \\ \hline
\begin{tabular}[c]{@{}c@{}}Implication/\\ Conditional\end{tabular} & $\to$ & If \\ \hline
Biconditional & $\leftrightarrow$ & If and Only If \\ \hline
NAND & $\uparrow$ & Not And \\ \hline
NOR & $\downarrow$ & Not Or \\ \hline
XOR & $\oplus$ & Exclusive Or \\ \hline
\end{tabular}
\end{table}

\subsubsection{Summary}

\begin{description}[leftmargin=!,labelwidth=7em]
  \item[Negation] Inverts the truth value
  \item[Conjunction] True when both statements are True
  \item[Disjunction] True when at least one of the statements are True
  \item[Biconditional] True when both statements have the same truth value
  \item[Implication] False if First and Second Statement are True and False respectively, otherwise all configurations are True
  \item[NAND] Negation of Conjunction
  \item[NOR] Negation of Disjunction
  \item[XOR] Negation of Biconditional
\end{description}

\newpage

\subsection{Truth Tables}

\begin{multicols}{2}
\centering

\begin{tabular}{>{\centering\arraybackslash}m{0.45\textwidth}}

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{Negation Truth Table}
\begin{tabular}{|c|c|}
\hline
\textbf{P} & \textbf{$\sim$P} \\ \hline
T & F \\ \hline
F & T \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{Conjunction Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\land$ Q} \\ \hline
T & T & T \\ \hline
T & F & F \\ \hline
F & T & F \\ \hline
F & F & F \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{Disjunction Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\lor$ Q} \\ \hline
T & T & T \\ \hline
T & F & T \\ \hline
F & T & T \\ \hline
F & F & F \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{Biconditional Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\leftrightarrow$ Q} \\ \hline
T & T & T \\ \hline
T & F & F \\ \hline
F & T & F \\ \hline
F & F & T \\ \hline
\end{tabular}
\end{minipage}
\\

\end{tabular}

% ------------

\begin{tabular}{>{\centering\arraybackslash}m{0.45\textwidth}}

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{Implication Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\to$ Q} \\ \hline
T & T & T \\ \hline
T & F & F \\ \hline
F & T & T \\ \hline
F & F & T \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{NAND Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\land$ Q} \\ \hline
T & T & T \\ \hline
T & F & F \\ \hline
F & T & F \\ \hline
F & F & F \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{NOR Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\downarrow$ Q} \\ \hline
T & T & F \\ \hline
T & F & F \\ \hline
F & T & F \\ \hline
F & F & T \\ \hline
\end{tabular}
\end{minipage}
\\

\begin{minipage}[c][12em][c]{\linewidth}
\centering
\captionof{table}{XOR Truth Table}
\begin{tabular}{|c|c|c|}
\hline
\textbf{P} & \textbf{Q} & \textbf{P $\oplus$ Q} \\ \hline
T & T & F \\ \hline
T & F & T \\ \hline
F & T & T \\ \hline
F & F & F \\ \hline
\end{tabular}
\end{minipage}
\\

\end{tabular}

\end{multicols}
