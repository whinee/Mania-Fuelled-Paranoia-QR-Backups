...
\documentclass[12pt]{codex} % tell the compiler to use the template
...
\title{Mania-Fulled Paranoia: How to Survive the Apocalypse by Printing Your Sh*t in QR Codes}     %note \\[1ex] is a line break in the title for nicer spacing if you wish to force it.
...
\subfile{./2_specs/dedication}        % include the dedication.tex file
\subfile{./2_specs/acknowledgements}  % include the acknowledgements.tex file
\tableofcontents                      % generate and include a table of contents
\listoffigures                        % generate and include a list of figures
\listoftables                         % generate and include a list of tables
\subfile{./2_specs/preface}           % include the preface.tex file
\end{romanpages}                      % end roman page numbering

\part{Mathematics}

\subfile{./2_specs/math/main}

\part{QR Code Standard}

\subfile{./2_specs/qr/main}
...
% \newcounter{preappendix}
% \setcounter{preappendix}{\value{chapter}}
% \appendix

\part{Appendices}

\subfile{./2_specs/appendix}

% End appendices: restore chapter numbering and hyperlink anchors
\clearpage
% \setcounter{chapter}{\value{preappendix}}
\renewcommand{\thechapter}{\arabic{chapter}}
\renewcommand{\thesection}{\arabic{chapter}.\arabic{section}}
\renewcommand{\appendixname}{} % optional cosmetic cleanup

% Fix hyperref anchors to avoid appendix namespace
\makeatletter
\let\@chapapp\chaptername      % revert "Chapter" text
\let\Hy@chapter\@chapter        % fix hyperref anchors
\makeatother
\input{./attributions}
